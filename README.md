...
- `requirements.in` 约束直接依赖，`requirements.txt` 精确锁定完整依赖树
- Docker Compose 本地编排
...
需要 Python 3.10+ 和 Node.js 24+。首次使用先安装依赖：
...
客户端只需在登录请求中通过 `X-Client-Type` 和 `X-Device-Name` 标识设备。刷新令牌每次使用后都会轮换，复用旧令牌会撤销同一设备的整个令牌族。
